...
The software half of the [pen plotter](../README.md). A Rhino Grasshopper
definition generates the linework and turns it into multi-pen G-code, and
Klipper macros handle registration, pen swaps and per-pen calibration.

The Rhino viewport is a digital twin of the canvas: the bed, the registered
sheet, the exclusion zone the offset pen creates, and the exact emission plan,
drawn at the pen widths and pressures that will actually lay the ink.

One thing from the hardware side matters throughout. The pen tip sits roughly
**52 mm in front of the nozzle datum**, and everything here works in pen
coordinates, applying that offset only when G-code is written.
...
| `../hardware/` | The pen mount: BOM, STLs, assembly drawing, [its own README](../README.md) |
