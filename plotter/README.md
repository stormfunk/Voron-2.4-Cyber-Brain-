...
- **This page** covers the Grasshopper pipeline, the Klipper macros and how to
  run a plot.
- **[`hardware/`](hardware/README.md)** covers the pen mount itself: how it
  works, the bill of materials, what to print and how it goes together.

## The hardware

![assembled pen-plotter mount](hardware/pen_mount/images/assembled-axo.png)

A removable pen toolhead built around the Archetype interface, replacing the
hotend fan for plotting. A short MGN9 rail constrains the pen carrier, twin
springs give compliant drawing pressure, and a self-centring collet keeps
different pen diameters on the same axis. The pen tip sits roughly **52 mm in
front of the nozzle datum**, and Grasshopper works in pen coordinates
throughout, applying that offset only when G-code is written.

**Bill of materials, printable files, the assembly drawing and the datum notes
are in [`hardware/`](hardware/README.md).** The rest of this page is the
software.
...
| `hardware/` | The pen mount: BOM, STLs, assembly drawing, [its own README](hardware/README.md) |
